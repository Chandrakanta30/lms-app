                        ...
                        <div class="card-header bg-white d-flex justify-content-between align-items-center py-3 flex-wrap">
                            ...
                            <div class="action-buttons">
                                ...
                                <a href="{{ route('manage-trainers', $training->id) }}" class="btn btn-sm btn-outline-primary">
                                    Trainers <span class="badge badge-primary ml-1">{{ $training->trainers->count() }}</span>
                                </a>
                        ...
                        <div id="collapse{{ $training->id }}" class="collapse" data-parent="#trainingAccordion">
                            <div class="card-body bg-light">
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md-3"><strong>Type:</strong> {{ ucwords(str_replace('_', ' ', $training->training_type ?? 'classroom')) }}</div>
                                    <div class="col-md-3"><strong>Start:</strong> {{ $training->start_date ? \Illuminate\Support\Carbon::parse($training->start_date)->format('d M Y') : 'N/A' }}</div>
                                    <div class="col-md-3"><strong>End:</strong> {{ $training->end_date ? \Illuminate\Support\Carbon::parse($training->end_date)->format('d M Y') : 'N/A' }}</div>
                                </div>
                                ...
                                <ul class="nav nav-pills mb-3" id="pills-tab-{{ $training->id }}" role="tablist">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link py-1 px-3" data-toggle="pill" href="#users-{{ $training->id }}">Trainees</a>
                                    </li>
                                    <li class="nav-item">
                                        <a class="nav-link py-1 px-3" data-toggle="pill" href="#docs-{{ $training->id }}">Documents</a>
                                    </li>
                                </ul>
                                        ...
                                        <div class="list-group list-group-flush">
                                            @foreach($training->steps as $step)
                                                <div class="list-group-item d-flex align-items-center justify-content-between">
                                                    <div>
                                                        <b class="text-primary mr-2">{{ $loop->iteration }}</b>{{ $step->name }}
                                                    </div>
                                                    <span class="text-muted small">{{ ucfirst($training->status ?? 'created') }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
                                            ...
                                            @forelse($training->trainees as $user)
                                                <div class="col-md-4 mb-2">
                                                    <div class="small p-3 rounded bg-light border">
                                                        ...
                                                        <span class="text-muted">{{ $user->designation->name ?? 'No designation' }}</span>
                                                    </div>
                                                </div>
                                            @empty
