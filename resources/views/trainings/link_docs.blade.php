            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <p class="card-description">Choose documents to include. The system will pull random questions from
                            each document based on the quota you set.</p>

                        <form action="{{ route('admin.modules.saveLinks', $module->id) }}" method="POST"
                            id="examConfigForm">
                            @csrf
                                            ...
                                            <tr>
                                                <td class="text-center">
                                                    <div class="form-check">
                                                        <input type="checkbox" name="docs[{{ $doc->id }}][selected]"
                                                ...
                                                <td>
                                                    ...
                                                    <small class="text-muted">{{ $doc->doc_number }} |
                                                        {{ $doc->doc_type }}</small>
                                                </td>
                                                <td class="text-center">
                                                ...
                                                <td>
                                                    <div class="input-group">
                                                        <input type="number" name="docs[{{ $doc->id }}][quota]"
                                                            class="form-control quota-input"
                                                            value="{{ $doc->questions_count == 0 ? 0 : $currentQuota }}"
                                                            min="{{ $doc->questions_count == 0 ? 0 : 1 }}"
                                                            max="{{ $doc->questions_count }}" placeholder="Qty"
                                                            {{ $isLinked ? 'required' : 'disabled' }}>

                                                        <div class="input-group-append">
                                                            <span class="input-group-text">
                                                                of {{ $doc->questions_count }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </td>
            ...
            const toggleQuotaState = (checkbox) => {
                ...
                if (checkbox.checked) {
                    ...
                    if (!quotaInput.value || parseInt(quotaInput.value) <= 0) {
                        quotaInput.value = parseInt(quotaInput.getAttribute('max')) > 0 ? 1 : 0;
                    }
                } else {
